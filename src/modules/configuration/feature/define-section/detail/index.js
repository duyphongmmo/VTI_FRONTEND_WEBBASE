...
import {
  DEFINE_SECTION_ENUM,
  DEFINE_SECTION_ENUM_MAP,
  DEFINE_SECTION_ENUM_OPTIONS,
  FIELDS_LOG_HISTORY_SECTION,
  FIELDS_LOG_HISTORY_SECTION_MAP,
...
function DefineSectionDetail() {
  ...
  const breadcrumbs = [
    ...
    {
      title: ROUTE.DEFINE_SECTION.DETAIL.TITLE,
    },
  ]
    ...
    actions.changeStatusDefineSection(
      {
        id: id,
        type:
          details?.status === DEFINE_SECTION_ENUM.ACTIVE
  ...
  const onSubmitDelete = () => {
    actions.deleteDefineSection(id, backToList)
  ...
  const onSubmitApprove = () => {
    actions.changeStatusDefineSection(
      {
        id: id,
        type: DEFINE_SECTION_ENUM.ACTIVE,
      },
      refreshData,
    )
    setIsOpenApprove(false)
  }
  const isActive = details?.status === DEFINE_SECTION_ENUM.ACTIVE
  const isInActive = details?.status === DEFINE_SECTION_ENUM.INACTIVE
  const isDraft = details?.status === DEFINE_SECTION_ENUM.DRAFT
  ...
  const actionBefore = () => {
    return (
      <>
        ...
        {/* </Guard> */}
        {isActive && (
          // <Guard code={FUNCTION_CODE.COST_CENTER_INACTIVE_SECTION}>
          <Button
            icon="active"
        ...
        {isInActive && (
          // <Guard code={FUNCTION_CODE.COST_CENTER_ACTIVE_SECTION}>
          <Button
            icon="inActive"
            onClick={() => setIsOpenChangeStatus(true)}
            color="success"
  ...
  const getHistory = () => {
    ...
    details?.histories?.forEach((item) => {
      if (item?.action === HISTORY_ACTION.CREATE) {
        histories.push({
          content: t('defineSection.history.create'),
        ...
        histories.push({
          ...
          content: () => {
            return (
              <>
                <Typography>{t('defineSection.history.update')}</Typography>
                <List sx={{ padding: 0 }}>
                  {(item?.fieldChanges || []).map((value, index) => {
                    let content = ''
                    switch (value?.changedField) {
                      case FIELDS_LOG_HISTORY_SECTION.STATUS:
                        content = `${t(
                          FIELDS_LOG_HISTORY_SECTION_MAP[value?.changedField],
                        )}: ${t(
                          DEFINE_SECTION_ENUM_MAP[value?.oldValue],
                        )} -> ${t(DEFINE_SECTION_ENUM_MAP[value?.newValue])}`
                        break
  ...
  return (
    <Page
      ...
      title={t('menu.defineSectionDetail')}
      loading={isLoading}
      onBack={backToList}
    >
      <HotKeys
        handlers={{
          onBack: backToList,
          // ...(canAccess(FUNCTION_CODE.UPDATE_SECTION)
          //   ? {
          //       onEdit: () => {
          //         history.push(
          ...
          //     }
          //   : {}),
          // ...(isDraft && canAccess(FUNCTION_CODE.DELETE_SECTION)
          //   ? { onDelete: () => setIsOpenDelete(true) }
          //   : {}),
          onEdit: () => {
            history.push(ROUTE.DEFINE_SECTION.EDIT.PATH.replace(':id', id))
          },
          onDelete: () => setIsOpenDelete(true),
        }}
      />
...
export default DefineSectionDetail
