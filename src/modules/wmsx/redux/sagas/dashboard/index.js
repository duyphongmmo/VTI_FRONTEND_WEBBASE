...
export const getPpmPage = (params) => {
  const uri = `/v1/dashboard/get-dashboard-page`;
  return api.get(uri, params);
};
...
function* doGetItemStockHistories(action) {
  try {
    const response = yield call(
      getDashboardItemStockHistories,
      action?.payload,
    );
    if (response?.statusCode === 200) {
...
function* doGetPpmChartList(action) {
  try {
    const response = yield call(getPpmChartList, action?.payload);
    if (response?.statusCode === 200) {
      const payload = {
        items: response.data.items,
        total: response.data.meta.total,
      };
      yield put(getPPMChartListSuccess(payload));
      if (action.onSuccess) {
        yield action.onSuccess();
      }
    } else {
...
function* doGetPpmPage(action) {
  try {
    const response = yield call(getPpmPage, action?.payload);
    if (response?.statusCode === 200) {
      const payload = {
        items: response.data.items,
        total: response.data.meta.total,
      };
      yield put(getPPMPageSuccess(payload));
      if (action.onSuccess) {
        yield action.onSuccess();
      }
    } else {
...
export default function* watchDashboard() {
  ...
  yield takeLatest(WMSX_GET_PPM_PAGE, doGetPpmPage);
}
